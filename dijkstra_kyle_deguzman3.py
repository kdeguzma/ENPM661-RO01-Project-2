...
def solver(width, height, sx, sy, gx, gy):
	...
	while not (current_node.xnode == gx and current_node.ynode == gy):
		...
		current_node = progress[0]
		count += 1
	#obsRemover(width, height)

	#visitednodes, allstates, path = dijkstraCheck(initial, node_network)
	print("\n")
	return allnodes
	#returnToThePast(allnodes)
...
goalx = 10 # int(input("Type x-coordinate goal point (mm): "))
goaly = 10 # int(input("Type y-coordinate goal point (mm): "))
